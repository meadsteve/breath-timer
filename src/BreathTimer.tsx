...
function circleRadiusRatio(
  ...
  switch (state) {
    case 'in':
      return 0.4 + (0.6 * runningFor) / states[state].duration;
    case 'out':
      return 1 - (0.6 * runningFor) / states[state].duration;
    case 'in_hold':
      return 1;
    case 'out_hold':
      return 0.4;
...
export default function BreathTimer({
  ...
  const [currentBreathState, setCurrentBreathState] =
    useState<BreathState>('in');
  ...
  const radius = `${
    circleRadiusRatio(runningFor, states, currentBreathState) * 80
  }vmin`;
  const mainColour = circleColour(runningFor, states, currentBreathState);
  const background = `radial-gradient(rgb(240, 240, 255), ${mainColour} 90%)`;
  const timeRemaining = states[currentBreathState].duration - runningFor;

  return (
    <div
      className="blue-circle"
      style={{ width: radius, height: radius, backgroundImage: background }}
    >
      <BreathText state={currentBreathState}>
        {states[currentBreathState].duration >= 2 ? (
